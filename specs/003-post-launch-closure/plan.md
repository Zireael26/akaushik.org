...
Close the backlog as six independently reviewable vertical slices, then run one integration/evidence slice. The MCP slice adds a small protocol adapter around existing content functions rather than a second service or mutable session store. It targets current stable MCP `2025-11-25` while supporting the bounded Streamable HTTP subset shared with `2025-06-18` and the required missing-header fallback `2025-03-26`: one same-origin JSON-RPC endpoint, negotiated initialization, `ping`, `tools/list`, `tools/call`, fallback-only batches capped at 32 calls, no server-sent-event stream, no session identifier, and no write-capable tool. Draft filtering is enforced inside the tool implementation even if a caller knows a draft slug. Production completion additionally requires an enforceable WAF rule of 60 `/api/mcp` requests per minute per source with a 60-second block.

The visual/content slices restore only already-designed work. Wanderer mounts through its existing raw-Three.js implementation, but route, CSS, and runtime media-query gates keep the companion home-only and absent on narrow or reduced-motion presentations; the SVG remains the desktop no-WebGL fallback. ClusterBid is promoted only after reconciling the owner-authored historical draft with the current sibling repository; all unsupported outcome/customer/production claims are removed and the case study uses a static, asset-free reel so publication cannot introduce a broken media request. Mobile navigation becomes a two-row, horizontally scrollable no-JavaScript surface with anchor clearance for the sticky header, and detail-page breadcrumbs extend the existing JSON-LD generators.

The platform slice pins Node 22, changes scheduled stats updates from direct pushes to lease-protected data-only pull requests generated from reviewed `main` code with the existing PAT, adds an executable production smoke probe and scheduled workflow, and adds a source-controlled favicon. Cloudflare email obfuscation, the MCP WAF rate-limit rule, and GitHub branch protection are live control-plane changes held until explicit confirmation and performed only after repository safeguards are merged and exact pre-state is captured. A final documentation/evidence slice reconciles current source-of-truth files, records fresh measurements, and annotates historical artifacts without rewriting old execution history.
...
| ------- | ---------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- | ---------------------------- |
| POST    | `/api/mcp` | JSON-RPC 2.0 object; `2025-03-26` also accepts arrays of 1–32 calls; `Accept` lists `application/json` and `text/event-stream`; explicit `MCP-Protocol-Version` is one of `2025-11-25`, `2025-06-18`, `2025-03-26`, while omission uses the mandated `2025-03-26` fallback; `Origin`, when present, must be canonical | JSON-RPC response for `initialize`, `ping`, `tools/list`, or `tools/call`; notification acknowledgement has no body | 200, 202, 400, 403, 406, 415 |
| GET, HEAD, PUT, PATCH, DELETE | `/api/mcp` | no body; optional supported protocol header; `Origin`, when present, must be canonical | Origin/protocol validation followed by a method-not-supported JSON response; no SSE stream | 400, 403, 405 |
...
- Malformed JSON returns `-32700`; parsed invalid envelopes return `-32600`; unsupported request methods and tools return `-32601`/`-32602`; known-tool input validation failures are actionable `isError: true` tool results; internal failures return redacted `-32603`. Valid notifications are one-way and receive HTTP 202 with no body. Array batches are accepted only for `2025-03-26` compatibility requests, capped at 32 calls, and omit every no-id member from the response.
...
| 3   | `app/api/mcp/route.ts`, `lib/mcp-http.ts`                                     | new                     | Expose the stateless same-origin Streamable HTTP adapter with centralized HTTP behavior.                            |
...
| 18a | `app/work/page.tsx`                                                           | modify                  | Keep work-index metadata aligned with the five published case studies.                                              |
| 18b | `app/opengraph-image.tsx`                                                     | modify                  | Keep the root share image's project list aligned with the five published case studies.                              |
| 18c | `app/layout.tsx`                                                              | modify                  | Keep root Open Graph copy aligned with the five published case studies; the same file also owns the Wanderer mount. |
...
| 21a | `components/seo/JsonLdScript.tsx`                                             | modify                  | Propagate the proxy's response nonce to every inline JSON-LD script.                                                |
| 21b | `components/seo/JsonLdScript.test.tsx`                                        | new                     | Lock JSON-LD nonce propagation and non-proxied fallback.                                                            |
...
| 35a | `.nvmrc`                                                                      | modify                  | Keep local version-manager selection aligned with Node 22 in CI and Vercel.                                         |
...
| 42a | `docs/ui-snapshots/2026-07-14/{home-desktop,home-mobile,work-clusterbid}.png` | new/derived             | Attach desktop/mobile UI receipts for navigation, Wanderer policy, and published ClusterBid presentation.           |
...
Use a contract-first fan-out. Subagents may prepare disjoint patches and proof in isolated worktrees, but they stop before commit/PR/merge. The main orchestrator integrates a linear stack in this exact order: spec → platform guardrails → MCP → UI/structured data → Wanderer → ClusterBid publication → tooling → docs/evidence. A branch ref is cut at each verified commit boundary; each PR targets `main` and merges only after its predecessor, so the next PR diff contains only its own segment. ClusterBid remains `draft: true` until its lane tests pass. Central ledgers (`docs/ROADMAP.md`, `docs/CHANGELOG.md`, spec checkboxes, snapshots) are owned only by the final integration lane.

Cloudflare email obfuscation is disabled only after the repository contact probe exists; the MCP WAF rule is created only after the endpoint deploys; GitHub branch protection is applied only after the bot-PR stats workflow reaches `main`. Each mutation additionally requires explicit operator confirmation, then exact pre-state and reversal capture. Repository changes ship independently while held control criteria remain open.

Every implementation commit is independently buildable. The integrated proof pass runs on the final stacked head before implementation PRs open. If that pass finds a defect, the orchestrator applies the fix to the owning lane before cutting branch refs (or appends an owning-lane fix and rebuilds every dependent ref), then reruns exact-head gates. Rollback is subsystem-specific merge-commit revert; ClusterBid can be reverted to draft independently and Wanderer can be removed at its single mount.

## 6. Test strategy

| Spec success criterion                                               | Test / proof                                                                                                                                                                                                       | Level                              | Fixture                                                                                                                                                                      |
| -------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| MCP protocol, version interoperability, rate limit, and draft safety | `lib/mcp.test.ts`; `e2e/mcp.spec.ts`; `npx -y @modelcontextprotocol/inspector@0.22.0 --cli <endpoint> --transport http --method tools/list`; Inspector `tools/call` probes for both tools; WAF setting/429 receipt | unit + integration + production    | published `neev`; cross-collection known draft slug; current/prior/fallback protocol revisions; official Inspector 0.22.0 performs initialize → initialized before list/call |
| MCP discovery/docs parity                                            | discovery/OpenAPI assertions in `e2e/agent-readiness.spec.ts`                                                                                                                                                      | integration                        | built app on localhost                                                                                                                                                       |
| Wanderer desktop/motion policy                                       | `e2e/canvas.spec.ts`; `e2e/reduced-motion.spec.ts`; attached desktop/mobile screenshots                                                                                                                            | e2e + visual                       | desktop Chromium/Firefox/WebKit, mobile projects, reduced motion                                                                                                             |
| ClusterBid corroborated publication                                  | placeholder grep, `lib/content.test.ts`, `e2e/work.spec.ts`, editorial diff review, attached detail-page screenshot                                                                                                | unit + e2e + review + visual       | current sibling repo plus owner-authored historical draft                                                                                                                    |
...
| Marquee/bundle/public-summary truth                                  | stale-dependency grep, five-case-study summary assertions, analyzer and Lighthouse snapshots                                                                                                                       | static + integration + performance | production build and Lighthouse artifacts                                                                                                                                    |
| Mobile nav and BreadcrumbList                                        | `e2e/home.spec.ts`; structured-data unit/page tests; attached 375px and desktop screenshots                                                                                                                        | unit + e2e + visual                | 375 px and desktop projects; writing/work fixtures                                                                                                                           |
...
The main orchestrator ships the linear stack as ordered `codex/` PRs; subagents stop at patches and review verdicts. Before each PR opens, run the process gate against that exact head and retain its receipt. Merge only through green CI with merge commits. After repository PRs merge and production is healthy: capture Cloudflare/Vercel/GitHub pre-state; disable Cloudflare Email Address Obfuscation and purge cache; create the MCP rate-limit rule; apply `main` protection; dispatch the production smoke; run the pinned official Inspector 0.22.0 through `tools/list`, `lookup_case_study`, and `get_availability`; and capture production/browser evidence. If verification regresses, restore the captured control value or revert only the responsible merge commit and hold dependents.

Control-plane rollback contract:

| Control                              | Required pre-state                                    | Target state                                                                            | Reversal                                                                                 |
| ------------------------------------ | ----------------------------------------------------- | --------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------- |
| Cloudflare Email Address Obfuscation | dashboard/API value and production decoder markers    | off; no decoder injection                                                               | restore captured value and purge cache                                                   |
| Vercel WAF MCP rate limit            | existing firewall/rate-limit rules                    | `/api/mcp`, 60 requests/min/source, 60-second block                                     | delete the named new rule or restore captured rule payload                               |
| GitHub `main` protection             | full protection payload; currently protection API 404 | require PR plus strict `verify`/`test`/`audit`, no force/delete, conversations resolved | restore captured protection payload, or DELETE protection only if restoring captured 404 |

## 8. Risks + mitigations

| Risk                                                        | Likelihood | Impact | Mitigation                                                                                                                                                            |
| ----------------------------------------------------------- | ---------- | ------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| MCP adapter diverges from client expectations               | medium     | high   | Target current stable plus explicit prior/fallback negotiation, bound fallback-only batches, use protocol fixtures and a real-client endpoint probe.                  |
...
| Persistent bot branch executes or preserves unreviewed code | low        | high   | Reset every run to exact `origin/main` before install/secret exposure, permit only the generated data diff, and update the branch with an exact-SHA force-with-lease. |
...
- **Decision:** Implement MCP directly against current stable `2025-11-25`, while supporting the bounded `2025-06-18` and `2025-03-26` compatibility subset.
  - **Why:** Two read-only tools and four methods do not justify another runtime dependency; explicit negotiation and fallback support provide current-client interoperability without adopting the draft 2026 transport.
  - **Rejected:** MCP SDK as a runtime dependency, single-version rejection, and the draft 2026 transport headers.
...
- **Decision:** Reinstate Wanderer only on the pose-driven home route and desktop with the existing crane and SVG.
  - **Why:** This resolves the recorded redesign follow-up while honoring the PRD's restrained-motion/mobile-density constraints and preventing the default hero pose from obscuring detail pages.

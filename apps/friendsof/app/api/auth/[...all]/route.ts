...
import { getPortalAuth, type PortalAuthEnv } from "@/lib/auth";

interface PortalBindings {
  AUTH_DB?: unknown;
  BA_SECRET?: string;
...
function portalEnv(): PortalAuthEnv & { isProduction: boolean } {
  const { env } = getCloudflareContext() as unknown as {
    env: PortalBindings & Record<string, string>;
  };
...
async function handleAuth(request: Request): Promise<Response> {
  ...
  const auth = await getPortalAuth(pEnv);

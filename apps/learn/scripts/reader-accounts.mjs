#!/usr/bin/env node
/**
 * Reader account CLI for learn.akaushik.org (private; never runs in a build).
 *
 ...
 * model: the course is readable by exactly the people whose accounts you
 * created here, and a leaked URL grants nothing.
 *
 * Commands:
 *   create            add one or more readers
 *   reset-password    rotate a password and kill that reader's sessions
 *   revoke-sessions   kill a reader's sessions, leaving the account
 *   remove            delete the account, its sessions, and its progress rows
 *   list              print usernames and created dates (no secrets)
 *
 * Credentials come from a root-managed private JSON file named by
 * READER_CREDENTIAL_FILE, which must be mode 0600. It holds either one reader
 * object or an array of them:
 *
 *   { "email": "...", "username": "...", "password": "...", "name": "..." }
 *
 * Adapted from apps/friendsof/scripts/owner-bootstrap.mjs. The properties kept
 ...
 *
 *   Wrangler (fallback). Set LEARN_AUTH_DB to the database *name* and run with
 ...
 * Optional env: PROGRESS_D1_ID / LEARN_PROGRESS_DB (required only by `remove`)
 */
...
if (!COMMANDS.includes(cmd)) {
  console.error(`usage: reader-accounts.mjs <${COMMANDS.join("|")}>`);
  process.exit(1);
}
...
} else if (!process.env.LEARN_AUTH_DB) {
  console.error(
    "Set CF_API_TOKEN (+ CF_ACCOUNT_ID, AUTH_D1_ID) for the REST transport, " +
      "or LEARN_AUTH_DB for the wrangler transport; refusing to run.",
  );
  process.exit(1);
} else {
  console.error(
...
const DB_NAMES = {
  auth: process.env.LEARN_AUTH_DB,
  progress: process.env.LEARN_PROGRESS_DB,
};
...
async function d1Batch(statements, databaseId = process.env.AUTH_D1_ID) {
  ...
  const name = databaseId === "progress" ? DB_NAMES.progress : DB_NAMES.auth;
...
if (readers.length === 0) {
  console.error("Credential file contains no readers; refusing to run.");
  process.exit(1);
}
...
function requireUsername(reader) {
  ...
  if (!username || username.length < 3) {
    console.error("A reader entry has an invalid username; refusing to run.");
    process.exit(1);
  }
  return username;
}
...
function requirePassword(reader) {
  ...
  if (password.length < 12) {
    console.error("A reader password is below minimum length 12; refusing to run.");
    process.exit(1);
  }
  return password;
}
...
for (const reader of readers) {
  ...
  if (cmd === "create") {
    ...
    if (!EMAIL_RE.test(email)) {
      console.error(`Reader ${JSON.stringify(reader.username)} has an invalid email; refusing to run.`);
      process.exit(1);
    }
  }
...
if (cmd === "revoke-sessions") {
  for (const reader of readers) {
    ...
    if (!row) {
      console.error(`${username}: no such reader; nothing revoked.`);
      process.exit(1);
    }
...
if (cmd === "reset-password") {
  for (const reader of readers) {
    ...
    const row = await d1One("SELECT id FROM user WHERE username = ?1", [username.toLowerCase()]);
    if (!row) {
      console.error(`${username}: no such reader; refusing to run.`);
      process.exit(1);
    }
...
if (cmd === "remove") {
  const progressTarget = USE_REST ? process.env.PROGRESS_D1_ID : "progress";
  if (USE_REST ? !process.env.PROGRESS_D1_ID : !DB_NAMES.progress) {
    console.error(
      "Set PROGRESS_D1_ID (REST) or LEARN_PROGRESS_DB (wrangler); " +
        "refusing to leave orphaned progress rows.",
    );
    process.exit(1);
  }
  for (const reader of readers) {
    ...
    const row = await d1One("SELECT id FROM user WHERE username = ?1", [username.toLowerCase()]);
    if (!row) {
      console.error(`${username}: no such reader; nothing removed.`);
      process.exit(1);
    }

    // Auth first. If the progress cleanup then fails, the reader is already
    // locked out and the leftover rows are inert until someone re-runs this.
    await d1Batch([
      ["DELETE FROM session WHERE userId = ?1", [row.id]],
    ...
    await d1Batch(
      [
        ["DELETE FROM lesson_progress WHERE account_id = ?1", [row.id]],
        ["DELETE FROM assignment_answer WHERE account_id = ?1", [row.id]],
        ["DELETE FROM assessment_answer WHERE account_id = ?1", [row.id]],
      ],
      progressTarget,
    );

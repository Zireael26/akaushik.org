...
import { getReaderAuth, type LearnAuthEnv } from "@/lib/auth";

interface LearnBindings {
  AUTH_DB?: unknown;
  BA_SECRET?: string;
...
function portalEnv(): LearnAuthEnv & { isProduction: boolean } {
  const { env } = getCloudflareContext() as unknown as {
    env: LearnBindings & Record<string, string>;
  };
...
async function handleAuth(request: Request): Promise<Response> {
  ...
  const auth = await getReaderAuth(pEnv);

/**
 * Security & header policy for learn.akaushik.org.
 *
 * Enforces:
...
/**
 * Marker a route sets on a response whose CSP it has already chosen — the
 * course atlases, which are self-contained HTML documents full of inline
 * script that the app's nonce CSP would kill. The Worker honours the route's
 * policy instead of overwriting it, and strips the marker before it ships.
 */
export const SANDBOXED_DOCUMENT_HEADER = 'x-learn-sandboxed-document';

/**
 * Policy for a sandboxed course document. It may run its own inline script and
 * nothing else: no network, no framing of anything, no form posts. The iframe
 * embedding it omits `allow-same-origin`, so this document also has an opaque
 * origin and cannot reach this app's cookies or storage.
 */
...
export function applySecurityHeaders(
  ...
  if (headers.get(SANDBOXED_DOCUMENT_HEADER) === '1') {
    headers.delete(SANDBOXED_DOCUMENT_HEADER);
    headers.set('content-security-policy', SANDBOXED_DOCUMENT_CSP);

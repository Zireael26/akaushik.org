/**
 * Cloudflare Worker wrapper for learn.akaushik.org.
 *
 * Wraps the OpenNext output with exact Host enforcement, a per-request nonce
 * CSP, and CSRF origin checks on auth mutations. Lifted from apps/friendsof;
 * see lib/learn-security.ts for the one behavioural difference, which is that
 * a route may claim its own policy for a sandboxed course document.
 */
import { applySecurityHeaders, buildCsp, generateNonce, isAllowedHost } from '../lib/learn-security';

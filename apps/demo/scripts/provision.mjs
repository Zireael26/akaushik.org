 ...
 *
 * Creating a D1 database that already exists is reported and skipped, so
 * re-running after a partial failure is safe. Nothing here deploys a Worker
 * or touches DNS. There is no R2 bucket: the demo serves no private files.
 */
...
for (const database of scope.d1_databases ?? []) {
  ...
  const placeholder =
    database.binding === 'AUTH_DB' ? `PROVISION_ME_AUTH_${suffix}` : `PROVISION_ME_CHAT_${suffix}`;

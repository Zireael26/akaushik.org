/**
 * Security & header policy for demo.akaushik.org.
 *
 * Enforces:
...
/**
 * Marker a route sets on a response whose CSP it has already chosen — a
 * self-contained HTML document full of inline script that the app's nonce CSP
 * would kill. The Worker honours the route's policy instead of overwriting it,
 * and strips the marker before it ships.
 */
export const SANDBOXED_DOCUMENT_HEADER = 'x-demo-sandboxed-document';

/**
 * Policy for a sandboxed document. It may run its own inline script and

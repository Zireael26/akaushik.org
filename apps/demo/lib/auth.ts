/**
 * Account authentication for demo.akaushik.org. better-auth 1.7.3 pinned.
 *
 * Lifted from apps/learn with the host and cookie name changed. The security properties below are the reason it was lifted rather
 ...
 *   disabled (disableSignUp) — demo accounts are
 *   created only by scripts/demo-accounts.mjs, so a leaked URL grants nothing.
 ...
 * - Cookies: literal `__Host-demo-session` (useSecureCookies:false removes
...
export const SESSION_COOKIE_NAME = "__Host-demo-session";
...
export type DemoAuth = Awaited<ReturnType<typeof buildAuth>>;

let cached: CacheEntry | null = null;

async function buildAuth(env: DemoAuthEnv) {
  ...
  return betterAuth({
    ...
    advanced: {
      ...
      cookies: {
        ...
        dont_remember: { name: "__Host-demo-remember" },
      },
...
export async function getDemoAuth(env: DemoAuthEnv): Promise<DemoAuth> {
...
/**
 * Server-only account identity. Derived from the session record, never from

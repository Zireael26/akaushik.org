/**
 * Cloudflare Worker wrapper for demo.akaushik.org.
 *
 * Wraps the OpenNext output with exact Host enforcement, a per-request nonce
 * CSP, and CSRF origin checks on auth mutations. Lifted from apps/learn;
 * see lib/demo-security.ts for the header policy.
 */
import { applySecurityHeaders, buildCsp, generateNonce, isAllowedHost } from '../lib/demo-security';
...
async function fetchHandler(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
  ...
  // Strict CSRF Origin enforcement for auth mutations and the demo chat
  // route at Worker edge before invoking KDF or proxying the chat stream.
  if (request.method === 'POST' && (url.pathname.startsWith('/api/auth/') || url.pathname === '/api/chat')) {
  ...
  // Remove untrusted client-supplied forwarded headers before they reach the app

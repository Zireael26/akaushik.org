...
test.describe('Work — cards and detail routes', () => {
  ...
  test('ClusterBid renders on both listings and its detail route', async ({ page }) => {
    ...
    // The H1 is no longer inside the article. `build-mdx-modules.ts` strips the
    // authored H1 + dek blockquote from case studies too (D4), because the
    // detail template renders both from frontmatter — so the heading lives in
    // the page header and the article holds body prose only. The page must
    // still have exactly one H1 and it must still be the study's name.
    await expect(page.getByRole('heading', { level: 1 })).toHaveText('ClusterBid');
    ...
    await expect(article.getByRole('heading', { level: 1 })).toHaveCount(0);
  ...
  test('Neev card links to /work/neev and the detail page renders', async ({ page }) => {
    ...
    // The Phase-2 arrangement is reversed: MDX no longer owns the title + dek.
    // The template renders both from frontmatter and the generator strips the
    // authored pair (D4), so the H1 is in the page header, not the article.
    await expect(page.getByRole('heading', { level: 1 })).toContainText('Neev');

...
**Measured (2026-07-04 audit):** 299,026 bytes (~292 KiB) script transferSize per Lighthouse `resource-summary` audit. The site is below the warning ceiling and still above the 150 KiB aspiration.

> The 400 KiB ceiling is a noise-control ceiling, not the product target. The product target remains 150 KiB; the current practical follow-up is to close the remaining raw-Three.js cost or formally accept a new target in a future ADR.

## Snapshots

- **2026-07-04, audit-remediation baseline:** `docs/bundle-snapshots/2026-07-04-bundle.md`. Script transferSize 299,026 bytes. Desktop and mobile runs cleared the configured Lighthouse assertions; local mobile performance cleared the configured score but missed the stricter LCP aspiration at ~3.6s.
...
2. `components/scene/WandererCrane*` is currently dormant via the disabled mount in `app/layout.tsx`; reinstatement must budget its runtime and fallback cost before merge.
...
1. Decide whether the raw-Three.js hero is worth the remaining ~140 KiB gap over the aspiration.
2. If the answer is no, prototype a smaller WebGL primitive or static/recorded hero treatment.
3. Keep mobile LCP as the next user-facing performance target; the local 2026-07-04 run cleared configured category thresholds but missed the stricter <2.5s LCP aspiration.
